    ...
    <div class="container">
        @if ($hasOverlayImage())
            <div x-data="{ isPlaying: false, embedUrl: '{{ $getEmbedSrc() }}' }" class="cursor-pointer group" aria-hidden="true">
                <div class="relative flex items-center justify-center" x-show="!isPlaying" x-transition
                     x-transition.delay.300ms>
                    {{ $getOverlayImageMedia(attributes: ['alt' => '', 'class' => 'w-full', 'loading' => 'lazy']) }}
                    <div class="absolute inset-0">
                        <div class="flex items-center justify-center h-full">
                            <button class="flex flex-col items-center justify-center before:transition-all before:duration-300 before:ease-in-out group-hover:before:bg-black/30 before:absolute before:bg-black/0 before:inset-0 group-hover:bg-black/0"
                                    @click="isPlaying = !isPlaying; $nextTick(() => { $refs.iframeElement.setAttribute('src', embedUrl) });">
                                <div
                                     class="relative z-10 text-white p-2 bg-black rounded-full bg-opacity-70 ">
                                ...
                                <div
                                     class="text-white mt-4 text-lg text-center px-2 bg-black rounded-lg bg-opacity-70">
